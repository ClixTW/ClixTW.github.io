...
網路上有很多關於這部分的教學，只是要成功架起來倒是不難，困難的點在於找到最方便維護的做法。主題可以下載解壓縮直接丟進 `themes` 目錄，也可以用 `Git Submodule` 方便更新；GitHub 的部分，可以只把 `public` 丟上去再手動部署，也可以把整個專案目錄放上去，再設定 GitHub Actions，每次 `push` 的時候自動部屬。分辨哪個做法比較高明，需要花點時間消化理解。
