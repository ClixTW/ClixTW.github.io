...
draft: false
---
Blackmagic Design 旗下的 DaVinci Resolve 是一款功能強大的影片剪輯軟體，最棒的是也提供了 Linux 版本！可惜僅提供特定發行版支援，且無法透過發行版的套件管理器安裝，還沒提供 .rpm 或 .deb 等包裝，需要使用官方腳本進行安裝。即便成功用腳本安裝好了，也會為未來系統維護帶來困難。因此這是我在 Linux 上安裝時唯一感到頭痛的軟體。
...
根據網路討論，Distrobox 有一個錯誤：停止並重新啟動容器後，容器內的 NVIDIA 運行環境符號連結會失效，導致容器中的軟體只在第一次啟動時能正常使用 CUDA。我們可以在啟動腳本加上確認機制，如果連結出問題就重建一遍，來暫時規避這個問題。
