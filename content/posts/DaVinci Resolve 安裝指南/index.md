...
draft: false
---
Blackmagic Design 旗下的 DaVinci Resolve 是一款功能強大的影片剪輯軟體，最棒的是有 Linux 版本！可惜僅提供特定發行版支援，且無法透過發行版的套件管理器安裝，還沒提供 .rpm 或 .deb 等包裝，需要使用官方腳本進行安裝。即便成功用腳本安裝好，也會為未來系統維護帶來困難。因此成為我唯一在 Linux 上安裝時感到頭痛的軟體。

幸好，現在有 Distrobox 這樣好用的工具，可以建立特定的發行版容器。這樣一來，就能在官方不提供支援的發行版上安裝，也可以避免安裝時往系統塞一大堆有的沒的套件。此外，有好心人維護了 Davincibox 這樣的腳本，大大降低了手動用 Distrobox 安裝的難度。

**但是！凡事都有個但是。** 如果你是中文使用者，當打字的時候，會驚訝地發現 Linux 版的 Resolve 不支援輸入法，所以沒辦法輸入中文。這個問題[在官方支援論壇有人提出來已經好幾年了](https://forum.blackmagicdesign.com/viewtopic.php?f=33&t=150886)，可能中文用戶太少，BMD 一直沒有處理。網路上有一些變通方法，但都是針對直接安裝才有效，透過容器安裝需要額外步驟。如果你是為了這個問題找到本文，恭喜你！我當初找遍網路都沒找到解法，花了好幾天才搞定呢。

解決完輸入法問題後，如果你是 NVIDIA 顯示卡的使用者，再次恭喜！安裝完的第一次啟動，一切都運作得那麼完美。直到隔天再次打開 Resolve，會發現怎麼卡卡的，甚至沒辦法正常輸出影片，檢查才發現 Resolve 找不到 CUDA 了。不對呀？昨天不是好好的嗎…原來[只有第一次啟動容器才能正確認到 CUDA，第二次就會找不到了](https://github.com/zelikos/davincibox/issues/154)。還好一樣有些變通方法，當初也是花了好大的功夫才找到問題點並解決。
...
*以下內容皆在 [Bluefin](https://projectbluefin.io/) 上，以 DaVinci Resolve Studio 20.3.2 版本驗證過。*
...
根據網路上討論，由於 Resolve 在建構時沒有開啟 QT 的輸入法支援，因此沒辦法正常輸入中文。我們可以在 Distrobox 中安裝 `fcitx5-qt5`，借用套件中的 `.so` 檔案，符號連結過去給 Resolve 使用。
...
> [!note]
> 上游的 Distrobox 已經合併了[修復的 PR](https://github.com/89luca89/distrobox/pull/2034)，1.8.2.5 後的版本應該就不會遇到這個問題了。
...
這是透過 Distrobox 安裝最棒的好處，可以為容器指定家目錄，避免 Resolve 直接在使用者目錄放置資料。當然如果你不在乎，也完全可以跳過這個步驟。雖然可以靠指令完成，但這裡用 [DistroShelf](https://flathub.org/en/apps/com.ranfdev.DistroShelf) 來操作。
...
1. 下載簡轉繁字體，比如：
   - 繁媛黑體：[https://github.com/ayaka14732/FanWunHak](https://github.com/ayaka14732/FanWunHak)
   - 原俠正楷：[https://github.com/tonyhuan/GuanKiapTsingKhai](https://github.com/tonyhuan/GuanKiapTsingKhai)
   - 思源黑體簡轉繁：[https://www.ptt.cc/bbs/Android/M.1496671955.A.543.html](https://www.ptt.cc/bbs/Android/M.1496671955.A.543.html)
2. 放至 `容器家目錄/.local/share/fonts` 目錄中。
...
- 不支援 H.264 / H.265：免費版完全不支援 H.264/H.265，不只硬體加速，連播放都不行，需要轉換成 ProRes、DNxHR 等格式，或購買工作室版本。
  ...
  > [!note]
    > 即便是工作室版，似乎也不支援 AMD 顯示卡的 H.264/H.265 編解碼，購買前請自行確認官方最新資訊。